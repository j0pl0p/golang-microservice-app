...
Я только бекенд успел сделать. Ну хотя бы апишка работает. Для скачивания проекта воспользуйтесь git clone. Выражения хранятся в СУБД, вместо gRPC я использую RabbitMQ. jwt не доделал(
